...
const {t} = useTranslation();
  return (
    ...
    <div className="flex text-center justify-between ml-4 w-[85%]" onClick={() => toggle(i)}>
      <h2 className='text-[14px] nm:text-[20px]'>{t('faq:q2')}</h2>
    ...
    <div className={selected === i ? 'content show' : 'content'}>{t('faq:a2')} <span className='cursor-pointer font-bold underline' onClick={() => window.open('https://www.youtube.com/watch?v=VymZkUd9eO8')}>{t('tutorial')}</span></div>
  </div>
  )
}

export default FAQP2;